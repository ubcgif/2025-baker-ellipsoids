...
def test_symmetry_prolate_oblate():
    ...
    for i in range(19):
        np.testing.assert_allclose(np.sqrt(ogn[i]**2 + ogu[i]**2), np.sqrt(ogn[i+1]**2 + ogu[i+1]**2))
        np.testing.assert_allclose(np.sqrt(pgn[i]**2 + pgu[i]**2), np.sqrt(pgn[i+1]**2 + pgu[i+1]**2))
        
        
def test_opposite_planes():
    
    e, n, u1 = vd.grid_coordinates(region = (-10, 10, -10, 10), spacing = 1, extra_coords= 5)
    e, n, u2 = vd.grid_coordinates(region = (-10, 10, -10, 10), spacing = 1, extra_coords=-5)
